# awesome-python
Awesome Python Tools and Resources

## Favorite Packages
### General
* https://pypi.org/project/Django/
* https://pypi.org/project/fastapi/
* https://pypi.org/project/fastapi-security/
* https://pypi.org/project/retry/
* https://pypi.org/project/httpx/
* https://pypi.org/project/structlog/
* https://pypi.org/project/cryptography/
* https://pypi.org/project/validators/

### CLI
* https://pypi.org/project/fire/
* https://pypi.org/project/click/

### Data Models, Input Validation, Configuration
* https://pypi.org/project/pydantic/
* https://pypi.org/project/hydra-core/
* https://pypi.org/project/PyYAML/

### ML, DataFrames, Notebooks
* https://pypi.org/project/pandas/
* https://pypi.org/project/polars/
* https://pypi.org/project/pyspark/
* https://pypi.org/project/delta-spark/
* https://pypi.org/project/ipykernel/
* https://pypi.org/project/ipywidgets/
* https://pypi.org/project/nbformat/
* https://pypi.org/project/nbconvert/
* https://pypi.org/project/nbstripout/
* https://pypi.org/project/databricks-sdk/

### CI/CD
* https://pypi.org/project/pre-commit/
* https://pypi.org/project/ruff/
* https://pypi.org/project/black/
* https://pypi.org/project/pyright/
* https://pypi.org/project/python-semantic-release/
* https://pypi.org/project/radon/
* https://pypi.org/project/pytest/
* https://pypi.org/project/coverage/
* https://pypi.org/project/packaging/
* https://pypi.org/project/toml/
* https://pypi.org/project/GitPython/
* https://pypi.org/project/PyGithub/
* https://pypi.org/project/pdoc3/
* https://pypi.org/project/pdoc/
* https://pypi.org/project/pymarkdownlnt/

#### Security
* https://pypi.org/project/pysast/
* https://pypi.org/project/safety/
* https://pypi.org/project/semgrep/
* https://pypi.org/project/checkov/
* https://pypi.org/project/n0s1/
* https://pypi.org/project/bandit/

## Package and Environment Managers
* https://github.com/astral-sh/uv (can replace poetry, venv, cargo, etc.)
* https://github.com/python-poetry/poetry

## Other Awesome Repos
* https://github.com/dylanhogg/awesome-python
* https://www.google.com/search?q=site%3Agithub.com+awesome+python&as_qdr=y1
* https://github.com/Svenskithesource/awesome-python-re reverse engineering tools
* https://github.com/sfermigier/awesome-functional-python

## PEPs and Style Guides
* https://google.github.io/styleguide/pyguide.html

